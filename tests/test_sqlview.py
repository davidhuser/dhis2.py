...
"""
    responses.add(
        responses.GET, "{}/data.csv".format(sql_view_view), body=r, status=200
    )
    ...
    for index, row in enumerate(
        api.get_sqlview(SQL_VIEW, True, criteria={"name": "0-11m"})
    ):
...
"""
    responses.add(
        responses.GET, "{}/data.csv".format(sql_view_query), body=r, status=200
    )

    expected = [
        {"dataelementid": "1151060", "name": "Inpatient cases", "valueType": "INTEGER"},
        {
            "dataelementid": "1151042",
            "name": "MNCH vacuum/forceps delivery",
            "valueType": "INTEGER",
        },
    ]
    for index, row in enumerate(
        api.get_sqlview(SQL_VIEW, var={"valueType": "INTEGER"})
    ):
        assert isinstance(row, dict)
        assert row == expected[index]
