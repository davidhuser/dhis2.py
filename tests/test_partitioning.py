...
def test_post_partitioned(api, payload, threshold, expected):
    ...
    for index, p in enumerate(api.post_partitioned(endpoint=endpoint, json=payload, thresh=threshold)):
        assert p.status_code == 200

    assert len(responses.calls) == len(expected)


@pytest.mark.parametrize("payload_invalid", [
    {
        "dataElements": [1, 2, 3, 4, 5, 6, 7, 8],
        "organisationUnits": []
    },
...
def test_post_partitioned_threshold_invalid(api, threshold):
    ...
    with pytest.raises(exceptions.ClientException):
        for _ in api.post_partitioned(endpoint='metadata', json=payload, thresh=threshold):
            continue
